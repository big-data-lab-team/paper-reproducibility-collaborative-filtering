...
The number of files selected for every subject is randomly selected in a 
uniform distribution with parameters \textit{\textbf{a}} and \textit{\textbf{b}} where \textit{\textbf{a}} and \textit{\textbf{b}} are set as follows:\\
\textit{\textbf{a}} : The minimume random of file index that could be selected for a subject\\ 
\textit{\textbf{b}} : The highest possibility of file index that could be selected for a subject\\
$\mathbf{\alpha}$ : Training ratio\\
\textit{\textbf{Nf}} : Index file which means all files with lower and equal to this number should be sampled into the training set.
\[
  f(\alpha;p) =
  \begin{cases}
	  R(0,Nf)      & \text{if $\alpha \leq 0.5$ }, \\
	  R(\alpha,Nf),  
	  a = (2\alpha - 1) Nf & \text{if $\alpha > 0.5$}.
  \end{cases}
\]
