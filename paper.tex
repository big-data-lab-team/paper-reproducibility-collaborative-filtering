...
a reproducibility crisis~\cite{baker2016there}. Infrastructural 
characteristics play an important role in making experiments 
reproducible, among other factors, such as data and code sharing. For 
instance, in 
...
may also trigger corrections not otherwise required. Some 
branches of the pipeline may also be executed only for specific 
acquisition 
parameters. These remarks are 
consistent with recent findings that showed that variations in the 
...
subjects have the same amount of images.
 ...
 vary at all, while other ones vary in every block. Such 
...
However, only 3 methods provide accuracy values above 0.85 
...
which slightly degrades the accuracy compared to 15 or 20.
...
Instead, the processing of all the subjects should be initiated, and it 
should be terminated in a random uniform way, assuming that files are 
uniformly produced throughout the execution. On more complex matrices, 
such as the synthetic dataset studied here, the training ratio required 
