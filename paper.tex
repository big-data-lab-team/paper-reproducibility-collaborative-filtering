...
matrix represents the ratings of items, represented by columns, by 
users, represented by rows. Ratings might be explicit, when users input 
ratings through a dedicated system, or implicit, when users' behaviors 
are analyzed to estimate their preference toward specific items. A 
complete introduction to collaborative filtering is provided 
in~\cite{mmdsbook}.

Several methods have been proposed to implement collaborative 
filtering. Item-item collaborative filtering~\ref{item-item} predicts 
the rating of item $i$ by user $u$ from the ratings of items similar to 
$i$ by user $u$. Likewise, user-user collaborative 
filtering~\ref{user-user} predicts the rating of item $i$ by user $u$ 
from the ratings of item $i$ by users similar to $u$. Both methods
have been used extensively for e-commerce applications.

A third class of methods, which we will apply to our problem, is based 
on the factorization of the utility matrix to estimate latent factors 
with which users and items are represented. This method is described 
in~\cite{koren} and became famous as it allowed its authors to win the 
