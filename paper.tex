...
We formulate the problem as collaborative filtering, with 
constraints on the construction of the training set. We propose 6 
different strategies to build the training set, and we evaluate them on 2 
...
respect the time constraints that stem from the order in which files 
...
\todo{Complete the introduction and improve the writing}
...
We assume that the order on files is consistent across subjects. $U_{i, 
j}$ measures the reproducibility of file $i$ produced during the 
processing of subject $j$ in two conditions, for instance two different 
...
average of the distribution is $\alpha N_{f}$:
\begin{enumerate}
...
versions of the CentOS operating system (5.\todo{?}, 6.8 and 7.2), using the
...
over 5 repetitions. \todo{Report model parameters, including number of 
factors.} Due to sampling issues, it is possible that the actual 
...
accuracy of the dummy classifier is close to 0.5 for all subject types. Random 
