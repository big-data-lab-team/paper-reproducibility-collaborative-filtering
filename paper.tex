...
\begin{abstract}
With the rise of the reproducibility crisis in several scientific 
domains, evaluating the computational reproducibility of data analysis 
pipelines has become a critical issue. It is, however, a cumbersome 
process for analyses that involve data from large populations of 
subjects, due to the computational and storage requirements. 
Nevertheless, executions of a given analysis pipeline are not 
independent, as they are obtained from the same program. We leverage 
this observation to propose a method to predict the computational 
reproducibility of data analysis pipelines in large population studies. 
We formulate the problem as a collaborative filtering problem with 
constraints on the construction of the training set. We propose 6 
different strategies to build the training set and evaluate them on 2 
datasets, a synthetic one modeling a population with a growing number 
of subject types, and a real one obtained on neuroinformatics 
pipelines. Results show that one sampling method, ``Random File Numbers 
(Uniform)" is able to predict computational reproducibility in 
pipelines with a good accuracy. We also analyze the relevance of 
including file and subject biases in the collaborative filtering model. 
We conclude that the proposed method is able to speed-up reproducibility 
...
the operating system or multithreading play an important role. In neuroinformatics, our 
...
due to the computational and storage requirements of data analysis pipelines.
...
Magnetic Resonance Imaging (MRI) and Electroencephalography (EEG). The number of files 
...
15 hours, with input ranging from 100~MB to 15~GB per subject, and 
outputs ranging from 1~GB to 500~GB. Such variations impact the 
reproducibility of a given pipeline across subjects. 

Our goal is to predict the outcome of reproducibility evaluations in a 
large population of subject from a reduced set of pipeline executions. 
More precisely, we aim at predicting whether a particular file produced 
by an analysis pipeline will be identical across execution conditions, or if it will contain reproducibility \emph{errors}. 
...
outside of its initial application domain, for instance 
~\cite{feng2013efficient}. 
...
\end{enumerate}
The paper ends with a discussion about \todo{x} and a conclusion.

\section{Method}

\subsection{Problem formulation}

The pipeline to be evaluated is represented by a matrix $U$ of size $N_f \times N_s$
where columns represent data coming from different subjects and rows represent 
the files produced by the pipeline execution. Subjects are unordered, 
i.e., columns may be reordered arbitrarily, but files are ordered, for 
...
represents the 
reproducibility of file $i$ produced during the processing of subject 
$j$ in two conditions, for instance two different operating systems. In our 
experiments, we restrict $U_{i,j}$ to be boolean, but the same prediction method
can be applied for real values. 

Our goal is to predict the test set $\mathbb{T'}$ of missing values of 
$U$ from a training subset $\mathbb{T}$ of known ones. In contrast with the traditional collaborative filtering 
problem, (1) we have control over the construction of the training set, and (2)
the construction of the training set is constrained by the order on the matrix rows, which is formalized as follows: 
\begin{equation}
\begin{array}{l}
...
pipeline matrix described previously.
...
Apache Spark's MLLib version 2.3.0, which uses ALS.
...
be very detrimental for more complex datasets such as the synthetic one used here.

% Other possible examples
The method described in this paper and applied to reproducibility 
studies could be used to predict the outcome of other deterministic, 
hidden, parameterized, time-dependent processes, for instance markers 
of chronicle disease activity.
...
the evaluated pipeline, it is hidden since we consider the pipeline as a black box (we 
...
\todo{Discussion on points that are not part of constant lines.}

\todo{Show an overlap to see where the error is located.}

RFNTS and RFNTL could be used when the matrix is more  uncertain at the beginning or end of the pipeline.

RS and RFNU tends to become the same for high training ratios.
