...
~\cite{feng2013efficient}. The pipeline is represented by a matrix 
where columns represent different parametrizations and rows represent 
the files produced by the execution. Parmeterizations may represent 
different input data sets or parameter values, on which we do not 
assume an order. On the contrary, an order is defined on the files, 
from their last modification time in a sequential execution. We assume 
that files are produced in the same order regardless of the 
parametrization, which is not always true, in particular when pipelines 
are multi-threaded, but in general reasonable \todo{explain "local" 
truth of this assumption}. This order imposes a constraint on the 
construction of the training set since files
