...
\todo{Explain this better}The training set sampling in this method is quite simillar to rows method.
The key difference lies in the way that the next subject is selected randomly. In this respect, subjects' files
are selected by coming their turn which is defined according to their generation time in the pipeline.
\begin{figure}
  \includegraphics[width=\columnwidth]{figures/5vs7_random-real_0.3_training_plot.png}
...
(Or: This method aims to sample some of the files which are almost generated at the end of the pipeline).
...
This ununiform distribution targeting to sample files which are generated 
almost the end of pipeline by meeting the closest training ratio  to the requested one by user.
In this approach at laest some of the first generated files of the pipelines are set in the training for each subject and there is no possibility of not having more than the first generated file for a subject. 
\subparagraph{smallest-a}
% For every method, show a training matrix to illustrate for a given ratio. 
This approach is almost the same as largest-a but the only difference is that there is probability of not having more than one file of a subject in the sample training set. 
