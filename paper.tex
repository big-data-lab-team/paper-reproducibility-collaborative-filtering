...
test set only contains positive elements \todo{Soudabeh: check matrix to confirm}). The average sensitivity and specificity values 
...
in the ALS optimization for the RFNU method and the synthetic dataset. We do not find a 
clear effect of the number of factors on the accuracy.
As it shown, number of factors does not have effect on accuracy specially for the training ratios less than 50\% .However, for the training ratios  over than that having less than 3 factors can dramatically effect the accuracy. Notably at training ratio of 90\% the major accuracy gap is observed between the compute accuracy with 2 factors and the 3 factors; 2 factors accuracy is about 40\% lower than 3 factors accuracy. 
...
Figure~\ref{fig:iterations} illustrates the impact of the number of iterations used to minimizing the error of matrix factorization for the RFNU method and synthetic dataset with 50 factors. As it shown, the increase in the number of iterations have slight effect on the accuracy except in the low training ratio of 10\% where maximum 20 iterations deteriorates the accuracy by 20\% . Also in the high training ratio such as 80\% and 90\% where 2 iterations leads to lower accuracy.   
\begin{figure}[h]
...
3 and iteration of 4. However, such studies could not be conducted by processing only half 
...
\todo{Soudabeh: push your code to reprotools. Tristan: release and check.}
